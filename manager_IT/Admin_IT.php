...
$stmt = $mysqli->prepare("
    ...
    WHERE 
      (j.job_status = 'waiting_approval' AND j.dep_id = '17')
      OR
      (j.job_status = 'in_progress' AND j.mgrapprove = ?)
      OR
      (J.job_status = 'waiting_approval' AND j.emp_id = 'A1262')
...
$Team = [
    ...
    ['id' => 'A1338', 'name' => 'Sitikorm'],
    ['id' => 'A0678', 'name' => 'Chuleeporn'],
    ['id' => 'A1430', 'name' => 'Naret']
];
    ...
    <header>
        ...
        <header class="bg-white shadow-md">
            <div class="container mx-auto px-4 md:px-6 py-2 flex justify-between items-center">
                <h1 onclick="window.location.href='../php/dashboard.php'" class="text-2xl font-bold text-green-500 hover:text-green-600 transition-transform transform hover:scale-105">
                ...
                <nav class="flex space-x-4 md:space-x-6">
                    <ul class="flex flex-wrap justify-center sm:justify-start space-x-2 sm:space-x-6">
                        <li class="flex-1 sm:flex-none">
                            <a href="../php/dashboard.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                HOME
                                <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                            </a>
                        </li>
                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] >= 2 && $_SESSION['emp_id'] !== 'A1263'): ?>
                            <li class="flex-1 sm:flex-none">
                                <a href="../manager/Admin.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                    APPROVED
                                    <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                                </a>
                            </li>
                        <?php endif; ?>
                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] && isset($_SESSION['emp_id']) && $_SESSION['emp_id'] == 'A1263'): ?>
                            <li class="flex-1 sm:flex-none">
                                <a href="../manager_IT/Admin_IT.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                    APPROVED
                                    <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                                </a>
                            </li>
                        ...
                        <?php if (isset($_SESSION['role']) && $_SESSION['role']): ?>
                            <li class="flex-1 sm:flex-none">
                                <a href="../php/checkrequest.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                    ACCEPTED
                                    <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                                </a>
                            </li>
                            <li class="flex-1 sm:flex-none">
                                <a href="../php/jobstatus.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                    job status
                                    <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                                </a>
                            </li>
                        <?php endif; ?>
                        <li class="flex-1 sm:flex-none">
                            <a href="../php/history.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                job HISTORY
                                <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                            </a>
                        </li>
                        <li class="flex-1 sm:flex-none">
                            <a href="../php/status.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                queue
                                <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                            </a>
                        </li>
                        <li class="flex-1 sm:flex-none">
                            <a href="../login/logout.php" class="relative text-gray-600 hover:text-green-500 transition-all duration-300 group transform hover:scale-105">
                                LOGOUT</a>
                            <span class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500 transition-all duration-300 group-hover:w-full"></span>
                        </li>
                </nav>
                <a href="../documents/tranform.php" class="create mt-3 sm:mt-0 px-4 py-2"><span>Create a request →</span>
        ...
        <div class="form-container" style="form-container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 shadow-md bg-white rounded-lg">
            ...
            transition duration-200 ease-in-out transform hover:scale-105 hover:shadow-lg hover:ring-2 hover:ring-green-400"
                                        data-details="<?= htmlspecialchars($job['job_details'], ENT_QUOTES) ?>">
        ...
        // }
        ?>
        <div id="TeamIT" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm hidden z-50">
            <div class="bg-white rounded-2xl shadow-xl w-11/12 max-w-md mx-auto p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Assignment</h2>
                <form id="assignForm" method="post" action="../manager_IT/sendmail_IT.php">
                    <input type="hidden" name="job_id" id="assignJobId">
                    <input type="hidden" name="action" value="approve">
                    <?php foreach ($Team as $emp): ?>
                        <div>
                            <label>
                                <input type="radio" name="assign_to" value="<?= htmlspecialchars(is_array($emp) ? $emp['id'] : $emp) ?>" required>
                                <?= htmlspecialchars(is_array($emp) ? $emp['name'] : $emp) ?>
                            </label>
                        </div>
                    <?php endforeach; ?>
                    <div class="mt-6 text-right">
                        <button type="submit" class="uppercase px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg shadow transition">
                            APPROVE
                        </button>
                        <button type="button" id="closeTeamModal"
                            class="uppercase px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg shadow transition">
                            CANCEL
                        </button>
                    </div>
        ...
        <script>
            ...
            // ฟังก์ชันเปิด Modal เลือกทีม IT และกำหนดค่า jobId ที่ต้องการ assign
            function openTeamModal(jobId) {
                document.getElementById('TeamIT').classList.remove('hidden'); // แสดง modal
                document.getElementById('assignJobId').value = jobId; // กำหนดค่า jobId ลงใน input ซ่อน
            }
            // เมื่อกดปุ่มปิด modal ให้ซ่อน modal ออกไป
            document.getElementById('closeTeamModal').onclick = function() {
                document.getElementById('TeamIT').classList.add('hidden'); // ซ่อน modal
            };
