...
// ---------- รายการ job_status ที่ไม่ต้องการให้แสดง ----------
$jobstatus = ['completed', 'cancel', 'rejected'];
// ---------- เตรียม SQL แบบ dynamic สำหรับ NOT IN (?, ?, ?) ----------
$placeholders = implode(',', array_fill(0, count($jobstatus), '?')); // ผลลัพธ์: '?,?,?' ตามจำนวน jobstatus

// ---------- นับจำนวนรายการทั้งหมดที่เข้าเงื่อนไข ----------
$sql = "SELECT job_id, job_details, detail_Rejected, job_status 
        FROM db_job 
        WHERE emp_id = ? 
        AND job_status NOT IN ($placeholders)
        LIMIT ? OFFSET ?";
...
<body class="bg-green-50 uppercase ">
  ...
  <div class="bg-white  shadow-lg w-full pt-6  flex flex-col items-center">
    ...
    <div class="flex justify-center items-center mb-6">
      <h2 class="text-2xl font-bold text-emerald-700 tracking-wide">📌 job status</h2>
    </div>
