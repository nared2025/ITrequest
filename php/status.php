...
$stmt = $mysqli->prepare("SELECT db_job.job_id, db_job.job_details,db_job.remark_it,db_job_assignments.duedate,db_job_assignments.assigned_emp,employee.emp_id,employee.emp_name FROM db_job_assignments INNER JOIN db_job ON db_job.job_id=db_job_assignments.job_id INNER JOIN employee ON db_job_assignments.assigned_emp=employee.emp_id WHERE job_status = 'pending' ORDER BY job_id ASC LIMIT ? OFFSET ?");
    ...
    <table class="min-w-full text-center table-auto bg-white">
      <thead class="bg-green-50 text-emerald-800 text-sm sm:text-base uppercase font-bold tracking-wide">
        <tr>
		  ...
		  <th class="px-4 sm:px-6 py-3">Support by</th>
...
$timestamp = strtotime(str_replace("/", "-", $duedate));

// สร้างวันที่และเวลาแบบไทย
$duedate = date("d", $timestamp) . "/" .
                  date("m", $timestamp) . "/" .
                  (date("Y", $timestamp) + 543) .
                  " เวลา " .
                  date("H:i", $timestamp) . " น.";
		  $supportby=$job['emp_name'];  
