...
if (!isset($_SESSION['emp_id']) || !isset($_SESSION['role']) || $_SESSION['dep_id'] != 17) {
  ...
  <header class="bg-white shadow-md">
    <div class="container mx-auto px-2 md:px-4 py-2
      ...
      <h1
        ...
        IT REQUEST
        <span class="text-gray-800 hover:text-gray-800">ONLINE</span>
      </h1>
        ...
        <ul class="flex flex-wrap justify-center sm:justify-start space-x-2 sm:space-x-6">
          ...
          <li class="flex-1 sm:flex-none">
            <a
              ...
              HOME
              <span
                class="absolute left-0 bottom-0 w-0 h-0.5 bg-green-500
      ...
      <div>
        <div class="bg-gradient-to-r from-green-100 via-blue-100 to-purple-100 rounded-xl shadow-lg p-8 mb-6 border border-gray-200">
          <div class="flex flex-col items-center justify-center space-y-3">
            ...
            <h1 class="text-3xl font-extrabold text-sky-600 tracking-tight drop-shadow">job ASSIGN</h1>
            <p class="text-gray-500 text-base text-center max-w-md">
              โปรดตรวจสอบรายละเอียดงานก่อนดำเนินการ
            </p>
          </div>
        </div>
      </div>
        ...
        <?php else: ?>
          <?php foreach ($jobs as $job): ?>
            <tr class="hover:bg-green-50 transition">
              ...
              <td>
                <div class="bg-gray-100 rounded-lg shadow p-2 text-gray-700 text-center">
                  <form method="post" action="../settingemail/updateduedate.php">
                    <input type="hidden" name="job_id" value="<?= htmlspecialchars($job['job_id']) ?>">                  
					<input type="datetime-local" id="datetime-local" name="datetime-local">	
                    <!-- <button type="submit" class="ml-2 bg-green-500 text-white px-2 py-1 rounded">บันทึก</button> -->
                </div>
              </td>
              <td>
                <div class="bg-gray-100 rounded-lg shadow p-2 text-gray-700 text-center">
                    <input type="hidden" name="job_id" value="<?= htmlspecialchars($job['job_id']) ?>">
                    <select name="remark">
                      <option value="16">PRE-ORDER</option>
                   
                    </select>
                    <button type="submit" class="ml-2 bg-green-500 text-white px-2 py-1 rounded">SAVE</button>
                  </form>
                </div>
              </td>
              <!-- <td>
